        ...
        if(path.size() > m_currentPathPointId)
        {
            ...
            if(distance__mm >= 100.0)
            {
                ...
                if(abs(angle) > 15.0)
                {
                    if(angle > 0.0)
                    {
                        emit SIG_ExecuteCmd(C_NApi_SerialCom::TURN_RIGHT, 1, angle);
                    }
                    else
                    {
                        emit SIG_ExecuteCmd(C_NApi_SerialCom::TURN_LEFT, 1, -angle);
                    }
                }
                else if(abs(angle) > 5.0)
                {
                    if(angle > 0.0)
                    {
                        emit SIG_ExecuteCmd(C_NApi_SerialCom::FORWARD_RIGHT, speed, distance__mm);
                    }
                    else
                    {
                        emit SIG_ExecuteCmd(C_NApi_SerialCom::FORWARD_LEFT, speed, distance__mm);
                    }
                }
                else
                {
                    emit SIG_ExecuteCmd(C_NApi_SerialCom::FORWARD, speed, distance__mm);
                }
            }
            else
            {
                // To be sure
                emit SIG_ExecuteCmd(C_NApi_SerialCom::STOP, 0, 0);
                emit SIG_ExecuteCmd(C_NApi_SerialCom::STOP, 0, 0);
...
void C_PathFinder::MapScene::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    case Qt::RightButton:
        ...
        emit ptParent->SIG_ExecuteCmd(C_NApi_SerialCom::STOP, 0, 0);
        emit ptParent->SIG_ExecuteCmd(C_NApi_SerialCom::STOP, 0, 0);
        break;
